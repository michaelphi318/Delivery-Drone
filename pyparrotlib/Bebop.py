...
class Bebop():
    ...
    def max_vertical_speed(self, speed):
        """
        Set max vertical speed in m/s
    ...
    def max_vertical_acceleration(self, a):
        if (a < 0 or a > 2.5):
            print("Error: %s is not valid acceleration. The acceleration must be between 0.5 and 2.5 m/s" % a)
            print("Ignoring command and returning")
            return

        command_tuple = self.command_parser.get_command_tuple("ardrone3", "PilotingSettings", "setAutonomousFlightMaxVerticalAcceleration")
        self.drone_connection.send_param_command_packet(command_tuple, param_tuple=[a], param_type_tuple=['float'])
        self.smart_sleep(1)
        # while (not self.sensors.max_vertical_speed_changed):
        #     self.smart_sleep(0.1)
    ...
    def max_rotation_speed(self,speed):
        ...
        command_tuple = self.command_parser.get_command_tuple("ardrone3", "PilotingSettings", "setAutonomousFlightMaxRotationSpeed")
        self.drone_connection.send_param_command_packet(command_tuple, param_tuple=[speed], param_type_tuple=['float'])
        self.smart_sleep(1)
        # while (not self.sensors.max_rotation_speed_changed):
        #     self.smart_sleep(0.1)
